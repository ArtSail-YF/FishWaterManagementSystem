...
export interface FeedingFilterValues {
...
interface FeedingFilterProps {
  onFilter: (values: FeedingFilterValues) => void;
  loading?: boolean;
}

const FeedingFilter: React.FC<FeedingFilterProps> = ({ onFilter, loading }) => {
  ...
  const activeFilterCount = [baseId, source, verifyStatus].filter(Boolean).length;
  ...
  return (
    ...
    >
      <Row gutter={[12, 12]} align="middle">
        ...
        <Col flex="auto">
          <Space>
            <Button type="primary" icon={<FilterOutlined />} onClick={handleSearch} loading={loading}>
              筛选
            </Button>
            <Button icon={<ReloadOutlined />} onClick={handleReset}>重置</Button>
            {activeFilterCount > 0 && (
              <Tag color="blue">{activeFilterCount} 个筛选中</Tag>
            )}
          </Space>
        </Col>
      </Row>
