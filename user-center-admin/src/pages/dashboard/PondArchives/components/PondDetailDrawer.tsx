...
import type { PondItem } from './PondGrid';
...
const PondDetailDrawer: React.FC<PondDetailDrawerProps> = ({ visible, pond, onClose, onDelete }) => {
  ...
  useEffect(() => {
    if (pond && visible) {
      form.setFieldsValue({
        ...
        status: pond.status,
        bottomType: '沙泥质',
        aerators: '3kW 浮子式 x 4, 1.5kW 纳米曝气 x 2'
      });
    }
    if (!visible) {
      setIsEditing(false);
  ...
  const showDeleteConfirm = () => {
    confirm({
      ...
      onOk() {
        onDelete(pond.id);
        message.success(`${pond.id} 档案已彻底删除`);
        onClose();
      },
    });
  };
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', paddingRight: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Title level={4} style={{ margin: 0 }}>{pond.id} 数字化档案</Title>
