...
import type { PondItem, PondSummaryStatsProps, ProductionLogItem } from '@/types';

// ================== 塘口管理 (Pond) ==================

export const MOCK_PONDS: PondItem[] = Array.from({ length: 18 }, (_, i) => ({
...
export const MOCK_POND_STATS: PondSummaryStatsProps = {
  totalPonds: 18,
  breedingCount: 12,
  emptyCount: 3,
  lockedCount: 1,
...
export const MOCK_FEEDING_LOGS: ProductionLogItem[] = [
  { id: '1', time: '2026-03-27 18:30:00', pondId: 'P001', type: 'feeding', content: '投喂 45.2kg (1.5mm 高蛋白)', operator: '系统自动', status: 'normal', category: 'pond', categoryName: 'P001', details: { feedType: '1.5mm 高蛋白', amount: 45.2, method: 'auto' } },
  { id: '2', time: '2026-03-27 18:15:00', pondId: 'P003', type: 'feeding', content: '投喂 38.5kg (1.5mm 高蛋白)', operator: '王大牛', status: 'high', category: 'pond', categoryName: 'P003', details: { feedType: '1.5mm 高蛋白', amount: 38.5, method: 'manual' } },
];

export const MOCK_MEDICINE_LOGS: ProductionLogItem[] = [
  { id: '1', time: '2026-03-27 10:30:00', pondId: 'P005', type: 'medicine', content: '使用 120g 聚维酮碘 (预防)', operator: '技术员-李工', status: 'locked', category: 'pond', categoryName: 'P005', details: { medicineName: '聚维酮碘', dose: 120, reason: '预防', withdrawalDays: 7, withdrawalRemaining: 7, status: 'locked' } },
  { id: '2', time: '2026-03-27 09:15:00', pondId: 'P012', type: 'medicine', content: '使用 50g 恩诺沙星 (肠炎)', operator: '技术员-李工', status: 'locked', category: 'pond', categoryName: 'P012', details: { medicineName: '恩诺沙星', dose: 50, reason: '肠炎', withdrawalDays: 15, withdrawalRemaining: 15, status: 'locked' } },
];
...
export const MOCK_REAL_TIME_WEATHER = {
  avgTemp: 26.4,
  maxWind: 4.2,
  status: 'normal',
  humidity: 75,
...
export const MOCK_DISASTER_INFO = {
  name: '台风',
  id: 'TY20260327',
  level: '红色预警',
  maxWind: 14,
...
export const MOCK_WATER_DATA_LIST = Array.from({ length: 18 }, (_, i) => ({
  id: `P${(i + 1).toString().padStart(3, '0')}`,
  name: `${i + 1}号池塘`,
  baseId: i < 5 ? 'B001' : i < 10 ? 'B002' : i < 15 ? 'B003' : 'B004',
  dissolvedOxygen: 4.5 + Math.random() * 2.5,
  waterTemperature: 22 + Math.random() * 5,
  pH: 7.0 + Math.random() * 1.5,
  status: i === 4 ? 'error' : i % 5 === 0 ? 'warning' : 'normal',
...
export const MOCK_POND_DETAIL_TREND = (id: string) => {
  const now = new Date();
  return Array.from({ length: 24 }, (_, i) => {
    const time = new Date(now.getTime() - (23 - i) * 60 * 60 * 1000);
    return {
      timestamp: time.toISOString(),
      dissolvedOxygen: 4.0 + Math.random() * 3.0,
      waterTemperature: 22.0 + Math.random() * 4.0,
      pH: 7.0 + Math.random() * 1.0,
    };
  });
};

export const MOCK_WATER_ALARM_LIST = [
  { id: '1', time: '2026-03-27 10:45:12', metric: '溶氧量', value: 2.1, threshold: 3.0, status: 'error', handle: '未处理' },
  { id: '2', time: '2026-03-27 09:30:00', metric: 'pH值', value: 8.5, threshold: 8.0, status: 'warning', handle: '未处理' },
  { id: '3', time: '2026-03-26 16:20:00', metric: '水温', value: 30.5, threshold: 28.0, status: 'warning', handle: '已处理' },
  { id: '4', time: '2026-03-27 08:15:00', metric: '溶氧量', value: 2.8, threshold: 3.0, status: 'warning', handle: '未处理' },
  { id: '5', time: '2026-03-27 07:45:00', metric: 'pH值', value: 6.5, threshold: 6.8, status: 'warning', handle: '已处理' },
];
